        ...
        with st.form("f_func", clear_on_submit=True):
            ...
            if ok:
                if not nome.strip():
                    ...
                    da.criar_funcionario({"nome": nome.strip(), "funcao": funcao.strip(), "telefone": telefone.strip(), "data_admissao": data_adm})
...
def page_usuarios():
            ...
            if ok:
                if not (nome.strip() and email.strip() and username.strip() and senha):
                    ...
                    okc, msg = da.criar_usuario({
                        "nome": nome.strip(), "email": email.strip(), "username": username.strip(),
                        "senha": senha, "perfil": perfil, "setor": setor.strip()
                    })
    ...
    with colB:
        ...
        with st.form("f_user_edit"):
            ...
            perfil = st.selectbox("Perfil", ["admin","comercial","producao","leitura"], index=["admin","comercial","producao","leitura"].index(urow.get("perfil","leitura")))
            ...
            if ok:
                ok2, msg2 = da.atualizar_usuario(uid, {
                    "nome": nome.strip(), "email": email.strip(), "username": username.strip(),
                    "perfil": perfil, "setor": setor.strip(), "ativo": ativo, "senha": senha
                })
